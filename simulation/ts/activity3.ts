...
let act3_canvas2: HTMLCanvasElement;
let act3_context2: CanvasRenderingContext2D;

let current_temp: number = 0;
let current_cap: number = 0;
...
let act3_probe: Chemistry.Custome_image;
let act3_left_pin: Chemistry.Custome_image;
let act3_right_pin: Chemistry.Custome_image;
let act3_pill_img: Chemistry.Custome_image;
//let act3_needle: Chemistry.act3_needle;

// em neddle angle
let act3_needle_angle: number = 150;
...
function activity3() {
	...
	let can4: HTMLButtonElement = <HTMLButtonElement> document.getElementById('offcanvasRight4');
	let offcanele: HTMLButtonElement = <HTMLButtonElement> document.getElementsByClassName('offcanvasbtn')[1];
	offcanele.innerHTML = `<i class="bi bi-table" style="font-size: calc(1vw + 12px);">`;
	offcanele.style.top = '7vw';
	can4.style.width = '75vw';

	pp.showtitle(
		`<p id="act3-exp-title">Obervation</p>`,
		3
	);
	pp.showdescription(
		`<div style="background-color: #f4ccccff; border-radius: 10px; border: black; padding: 5%; font-weight: 500; font-size: calc(1vw + 12px);">You need to take observations at t = 25, 78, 134, 172, 200 for Barium titatnate and for lead zirconate t = 29, 106, 168, 208, 273</p>`,
		3
	);

	pp.showdescription(
		`<div style="background-color: #f4ccccff; border-radius: 10px; border: black; padding: 2%; font-weight: 500; font-size: calc(1vw + 12px);">Observation Table</p> <br> <div id='obs-table' ></div>`,
		4
	);
	...
	canvas = <HTMLCanvasElement>document.getElementById('canvas-new');
	...
	act3_probe = new Chemistry.Custome_image(
		assembly,
...
function power_on() {
	let first_box: HTMLDivElement = <HTMLDivElement> document.getElementById('start-first');
	let dsp_box: HTMLDivElement = <HTMLDivElement> document.getElementById('display-box');
	let cntr: HTMLDivElement = <HTMLDivElement> document.getElementById('control-1');
...
function inc_temp() {
	...
	if(selected_material_index == 0 && current_index < 174) {
		current_index++;
		let inp1: HTMLInputElement = <HTMLInputElement> document.getElementById('c-dsp');
		let inp2: HTMLInputElement = <HTMLInputElement> document.getElementById('t-dsp');
...
function add_obs_reading() {
	...
	let parent: HTMLDivElement = <HTMLDivElement> document.getElementById('obs-table');
